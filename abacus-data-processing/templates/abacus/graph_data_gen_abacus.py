...
# Ensure local template helpers and skill-level scripts are importable without manual PYTHONPATH.
_SCRIPT_DIR = Path(__file__).resolve().parent
_SKILL_SCRIPTS_DIR = _SCRIPT_DIR.parent.parent / 'scripts'
for _candidate in (_SCRIPT_DIR, _SKILL_SCRIPTS_DIR):
...
# `19` is an auto mode resolved to a supported ABACUS basis profile at runtime.
DEFAULT_NAO_MAX = 19
DEFAULT_EFFECTIVE_NAO_MAX = 27
NAO_MAX = DEFAULT_EFFECTIVE_NAO_MAX
...
def parse_args():
    ...
    parser.add_argument(
        '--nao-max',
        type=int,
        default=DEFAULT_NAO_MAX,
        help='Maximum number of atomic orbitals. Supported values: 13, 15, 19(auto), 27, 40.',
    )
...
def configure_runtime(runtime_config):
    ...
    global NUM_PROCESSES, WORKER_THREADS, POOL_CHUNKSIZE, LMDB_COMMIT_INTERVAL, NAO_MAX
    ...
    requested_nao_max = int(runtime_config['nao_max'])
    NAO_MAX = resolve_requested_nao_max(requested_nao_max, DATA_DIRS)
    configure_basis_definitions(NAO_MAX)
...
AUTO_NAO_MAX = 19
AUTO_NAO_CANDIDATES = (27, 40)
STRU_SECTION_END_TOKENS = {
    'LATTICE_CONSTANT',
    'LATTICE_VECTORS',
    'ATOMIC_POSITIONS',
    'NUMERICAL_ORBITAL',
...
def _format_element_list(atomic_numbers) -> str:
    if not atomic_numbers:
        return '(none)'
    return ', '.join(_z_to_symbol(z) for z in sorted(set(int(v) for v in atomic_numbers)))


def _parse_atomic_numbers_from_stru(stru_path: str) -> set[int]:
    atomic_numbers: set[int] = set()
    in_atomic_species = False
    try:
        with open(stru_path, 'r', encoding='utf-8', errors='ignore') as f:
            for raw_line in f:
                ...
                if upper_token in STRU_SECTION_END_TOKENS:
                    break
                if ELEMENT_SYMBOL_PATTERN.match(token):
                    try:
                        atomic_numbers.add(int(Element(token).Z))
                    except Exception:
                        continue
    except Exception:
        return set()
...
def _collect_atomic_numbers(data_dirs: list[str]) -> set[int]:
    atomic_numbers: set[int] = set()
    for data_dir in data_dirs:
        stru_candidates = (
            os.path.join(data_dir, 'STRU'),
            os.path.join(data_dir, 'OUT.ABACUS', 'STRU'),
        )
        for stru_path in stru_candidates:
            if os.path.isfile(stru_path):
                atomic_numbers.update(_parse_atomic_numbers_from_stru(stru_path))
                break
    return atomic_numbers


def _missing_atomic_numbers(nao_max: int, atomic_numbers: set[int]) -> list[int]:
    basis_def = SUPPORTED_NAO_MAX[nao_max]
    return sorted(z for z in atomic_numbers if z not in basis_def)


def resolve_requested_nao_max(requested_nao_max: int, data_dirs: list[str]) -> int:
    if requested_nao_max == AUTO_NAO_MAX:
        atomic_numbers = _collect_atomic_numbers(data_dirs)
        if not atomic_numbers:
            fallback = AUTO_NAO_CANDIDATES[0]
            print(
                f'Warning: Auto NAO_MAX={AUTO_NAO_MAX} could not detect elements from STRU files. '
                f'Falling back to NAO_MAX={fallback}.'
            )
            return fallback

        for candidate in AUTO_NAO_CANDIDATES:
            missing = _missing_atomic_numbers(candidate, atomic_numbers)
            if not missing:
                print(
                    f'Auto NAO_MAX={AUTO_NAO_MAX} resolved to {candidate} '
                    f'for elements: {_format_element_list(atomic_numbers)}.'
                )
                return candidate
        ...
        for candidate in AUTO_NAO_CANDIDATES:
            missing = _missing_atomic_numbers(candidate, atomic_numbers)
            diagnostics.append(f'{candidate} missing [{_format_element_list(missing)}]')
        raise ValueError(
            'Auto NAO_MAX resolution failed for detected elements '
            f'[{_format_element_list(atomic_numbers)}]. '
            + '; '.join(diagnostics)
        )

    if requested_nao_max not in SUPPORTED_NAO_MAX:
        raise ValueError(
            f'Unsupported NAO_MAX={requested_nao_max}. '
            'Supported values: 13, 15, 19(auto), 27, 40.'
        )

    atomic_numbers = _collect_atomic_numbers(data_dirs)
    if atomic_numbers:
        missing = _missing_atomic_numbers(requested_nao_max, atomic_numbers)
        if missing:
            raise ValueError(
                f'NAO_MAX={requested_nao_max} is incompatible with elements '
                f'[{_format_element_list(missing)}]. '
                'Use --nao-max 19 (auto), 27, or 40.'
            )
    return requested_nao_max


def configure_basis_definitions(nao_max: int) -> None:
    global BASIS_DEF, BASIS_NUM
    if nao_max not in SUPPORTED_NAO_MAX:
        raise ValueError(
            f'Unsupported NAO_MAX={nao_max}. '
            'Supported values: 13, 15, 19(auto), 27, 40.'
        )
    BASIS_DEF = SUPPORTED_NAO_MAX[nao_max]
    BASIS_NUM = np.zeros((99,), dtype=int)
...
def resolve_sparse_file(scf_path: str, candidate_names) -> str:
    for name in candidate_names:
        path = os.path.join(scf_path, name)
        if os.path.isfile(path):
            return path
    candidates = ", ".join(candidate_names)
    raise FileNotFoundError(f"None of the sparse files exists under {scf_path}: {candidates}")


def resolve_h0_sparse_file(scf_path: str) -> str:
    preferred = 'data-H0R-sparse_SPIN0.csr'
    fallback = 'data-HR-sparse_SPIN0.csr'
    resolved = resolve_sparse_file(scf_path, (preferred, fallback))
    if os.path.basename(resolved) != preferred:
        print(f"Warning: {preferred} not found under {scf_path}. Falling back to {os.path.basename(resolved)}.")
    return resolved


def resolve_s_sparse_file(scf_path: str) -> str:
    preferred = 'data-S0R-sparse_SPIN0.csr'
    fallback = 'data-SR-sparse_SPIN0.csr'
    resolved = resolve_sparse_file(scf_path, (preferred, fallback))
    if os.path.basename(resolved) != preferred:
        print(f"Warning: {preferred} not found under {scf_path}. Falling back to {os.path.basename(resolved)}.")
    return resolved
...
def generate_graph(task: tuple) -> tuple:
    ...
    try:
        ...
        h0_sparse = ABACUSHS(resolve_h0_sparse_file(scf_path))
        if SKIP_DFT_HAMILTONIAN:
            h_sparse = None
        else:
            h_sparse = ABACUSHS(resolve_sparse_file(scf_path, ('data-HR-sparse_SPIN0.csr',)))
        s_sparse = ABACUSHS(resolve_s_sparse_file(scf_path))
...
def main():
    ...
    print(
        ...
        f'worker_threads={max(1, WORKER_THREADS)}, chunksize={chunksize}, nao_max={NAO_MAX}.'
    )
